...
int main(){
	//set<pair<std::string, pair<int, pair<int, int>>>> countryMedals;
	//set<pair<std::string, pair<int, pair<int, int>>>>::iterator it;

	set<pair<std::string, int>> countryGold, countrySilver, countryCooper;
	set<pair<std::string, int>>::iterator it1;

	char countryName[11234];
	...
	while(scanf("%[^\n]", countryName) != EOF){
		...
		scanf("%[^\n]", countryName);
		getchar();
		it1=countryGold.find(countryName);
